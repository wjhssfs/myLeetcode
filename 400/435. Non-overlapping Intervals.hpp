    ...
    int eraseOverlapIntervals(vector<Interval>& intervals) {
        ...
        for (int i = 0; i < (int)intervals.size()-1; ++i) {
            if (intervals[i+1].start < intervals[i].end) {
                int ci = 1, cj = 1;
                for (int j = i + 2; j < intervals.size() && intervals[j].start < intervals[i].end; ++j) ++ci;
                for (int j = i + 2; j < intervals.size() && intervals[j].start < intervals[i+1].end;++j) ++cj;
                if (ci < cj) swap(intervals[i], intervals[i+1]);
                ++nErase;
            }
        }
        return nErase;
