...
public:
    string findReplaceString(string S, vector<int>& indexes, vector<string>& sources, vector<string>& targets) {
        ...
        int lastEnd = 0;
        for (auto&& ind : sorted) {
            auto s = sources[ind.second];
            if (S.substr(indexes[ind.second], s.size()) == s) {
                r += S.substr(lastEnd, indexes[ind.second] - lastEnd);
                r += targets[ind.second];
                lastEnd = indexes[ind.second] + s.size();
            }
        }
        r += S.substr(lastEnd);
        return r;
