...
public:
    int findRadius(vector<int>& houses, vector<int>& heaters) {
        ...
        int minR = 0;
        while (i < houses.size()) {
            if (houses[i] <= heaters[j] || j == heaters.size() - 1) {
                minR = max(minR, heaters[j] > houses[i] ? heaters[j] - houses[i] : houses[i] - heaters[j]);
                ++i;
            } else if (houses[i] >= heaters[j+1]) {
                ++j;
            } else {
                minR = max(minR, min(heaters[j+1] - houses[i], houses[i] - heaters[j]));
                ++i;
            }
        }
        return minR;
