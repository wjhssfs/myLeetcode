...
public:
    int intersectionSizeTwo(vector<vector<int>>& intervals) {
        ...
        int result = 0;
        vector<int> results = {intervals[0][1] - 1, intervals[0][1]};
        for (int i = 1; i < intervals.size(); ++i) {
            if (intervals[i][0] <= results[results.size() - 2]) continue;
            else if (intervals[i][0] <= results.back()) {
                results.push_back(intervals[i][1]);
            } else {
                results.push_back(intervals[i][1] - 1);
                results.push_back(intervals[i][1]);
            }
        }
        return results.size();
